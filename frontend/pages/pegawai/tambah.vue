...
const loadMaster = async () => {
  master.value.agama = (await api.get('/agama')).data
  master.value.unit_kerja = (await api.get('/unit-kerja')).data
  master.value.jabatan = (await api.get('/jabatan')).data
  master.value.golongan = (await api.get('/golongan')).data
  master.value.eselon = (await api.get('/eselon')).data
}

onMounted(loadMaster)

const onInputNumber = (field, maxLength) => {
  form[field] = form[field].replace(/\D/g,'').slice(0, maxLength)
}

// ===== Tanggal lahir batas 20-59 tahun =====
const getMinDate = () => {
  const d = new Date()
...
const validate = () => {
  ...
  else if (!/^\d{5,}$/.test(form.nip)) { errors.nip = 'NIP harus angka minimal 5 digit'; valid = false }
  ...
  else if (!/^\d{10,}$/.test(form.no_hp)) { errors.no_hp = 'No HP harus angka minimal 10 digit'; valid = false }

  if (!form.npwp) { errors.npwp = 'NPWP wajib diisi'; valid = false }
  else if (!/^\d{15}$/.test(form.npwp.replace(/\D/g,''))) { errors.npwp = 'NPWP harus 15 digit angka'; valid = false }
...
const submit = async () => {
  ...
  try {
    await api.post('/pegawai', fd, { headers: { 'Content-Type': 'multipart/form-data' } })
    navigateTo('/pegawai')
  } catch (err) {
    error.value = 'Gagal menyimpan pegawai'
  }
...
<div class="max-w-5xl mx-auto p-8">
  <h1 class="text-3xl font-bold text-gray-800 mb-6 border-b pb-3">Tambah Pegawai</h1>
    ...
    <form @submit.prevent="submit" class="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div class="flex flex-col gap-5">
        <div>
          ...
          <input v-model="form.nip" @input="onInputNumber('nip',20)"
                 maxlength="20"
                 :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.nip ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']" />
        ...
        <div>
          ...
          <input v-model="form.nama"
                 :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.nama ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']" />
        ...
        <div>
          ...
          <input v-model="form.tempat_lahir"
                 :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.tempat_lahir ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']" />
        ...
        <div>
          ...
          <input type="date" v-model="form.tgl_lahir"
                 ...
                 :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.tgl_lahir ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']" />
        ...
        <div>
          ...
          <textarea v-model="form.alamat" rows="3"
                    :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.alamat ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']"></textarea>
      ...
      <div class="flex flex-col gap-5">
        <div>
          ...
          <select v-model="form.agama_id"
                  :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.agama_id ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']">
            <option value="">Pilih Agama</option>
            <option v-for="a in master.agama" :key="a.id" :value="a.id">{{ a.nama }}</option>
          </select>
        ...
        <div>
          ...
          <select v-model="form.unit_kerja_id"
                  :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.unit_kerja_id ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']">
            <option value="">Pilih Unit Kerja</option>
            <option v-for="u in master.unit_kerja" :key="u.id" :value="u.id">{{ u.nama }}</option>
          </select>
        ...
        <div>
          ...
          <select v-model="form.jabatan_id"
                  :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.jabatan_id ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']">
            <option value="">Pilih Jabatan</option>
            <option v-for="j in master.jabatan" :key="j.id" :value="j.id">{{ j.nama }}</option>
          </select>
        ...
        <div>
          ...
          <select v-model="form.golongan_id"
                  :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.golongan_id ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']">
            <option value="">Pilih Golongan</option>
            <option v-for="g in master.golongan" :key="g.id" :value="g.id">{{ g.nama }}</option>
          </select>
        ...
        <div>
          ...
          <select v-model="form.eselon_id"
                  :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.eselon_id ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']">
            <option value="">Pilih Eselon</option>
            <option v-for="e in master.eselon" :key="e.id" :value="e.id">{{ e.nama }}</option>
          </select>
        ...
        <div>
          ...
          <input v-model="form.tempat_tugas"
                 :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.tempat_tugas ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']" />
        ...
        <div>
          ...
          <input v-model="form.no_hp" @input="onInputNumber('no_hp',15)"
                 maxlength="15"
                 :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.no_hp ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']" />
          <p v-if="errors.no_hp" class="text-red-500 text-sm mt-1">{{ errors.no_hp }}</p>
        </div>

        <div>
          <label class="font-medium text-gray-700">NPWP</label>
          <input v-model="form.npwp" @input="onInputNumber('npwp',15)"
                 maxlength="15"
                 :class="['mt-1 p-3 border rounded-lg w-full focus:ring-2', errors.npwp ? 'border-red-500 ring-red-300' : 'border-gray-300 ring-blue-400']" />
      ...
      <div class="md:col-span-2 flex justify-between gap-5 mt-6">
        <button type="button" @click="navigateTo('/pegawai')"
